...
7. **`entities_without_entry_points`: 267.** All 267 are NPCs — 182 `Type: object` (vehicles, statues, machinery, decorations the engine treats as NPCs) and 85 `Type: person` (mostly cutscene-only or inaccessible characters, plus some sprite-classification quirks like door sprites grouped under `person`). Triaged and confirmed legitimate; not an extraction gap. CSV report in [tools/entities_without_dialogue.csv](../tools/entities_without_dialogue.csv) for reference. Generator: [tools/list_entities_without_dialogue.py](../tools/list_entities_without_dialogue.py).
